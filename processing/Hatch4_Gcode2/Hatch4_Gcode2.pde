...
float   whiteThreshold = 120; //120 
int valueContrast = 100; //60
int valueBrightness = 100; // -10
int valueBlur = 1;

  // spacing between lines in the hatching
  int     gridSpacingBlack = 2; //2
int     gridSpacingGray  = 6; //6
boolean stateHatching = false;
...
void setup() {
  ...
  String outFile = "GCODE/"+fileNoExt+".GCODE";
  ...
  opencv.brightness(valueBrightness);
  opencv.contrast(valueContrast);
  opencv.blur(valueBlur);
