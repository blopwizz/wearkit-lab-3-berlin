...
void osc_update() {
  if (v_resistivity < 200 && !ok_machine_busy && !ok_sequence_busy) {
    ...
    update();
    if (ok_print_debug) print_debug(" OSC DETECTED, PHOTO SHOT");

    previz_init();
    if (ok_print_debug) print_debug(" OSC DETECTED, PREVIZ");
  }
  if (ok_sequence_busy && ok_machine_ready && !ok_machine_busy && ok_previz_done) {
    machine_launch_task();
    ok_sequence_busy = false;
    if (ok_print_debug) print_debug(" OSC DETECTED, MACHINE");
  }
}
