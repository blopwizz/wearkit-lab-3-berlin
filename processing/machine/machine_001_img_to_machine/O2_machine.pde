...
void machine_send_instruction(String line) {
  ...
  case "G1": 
    ...
    machine_continueDraw(machine_pen_prev, machine_pen);
    machine_pen_prev.set(machine_pen);
    break;
  case "G0":
    machine_pen.x = float(instruction[1].substring(1));
    machine_pen.y = float(instruction[2].substring(1));
    machine_moveAndDraw(machine_pen_prev, machine_pen);
    machine_pen_prev.set(machine_pen);
    break;
  case "M03":
    pen_up();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
// UPDATE : GLINE
void machine_moveAndDraw(PVector p1, PVector p2) { 
  pushStyle();
  pushMatrix();
  translate(100, 100);
  scale(c_scale);
  ...
  popStyle();
  pen_up();
  machine_move_fast(p1.x, p1.y);
  pen_down();
  machine_move_abs(p2.x, p2.y);
  counterLines++;
}

void machine_continueDraw(PVector p1, PVector p2) { 
  pushStyle();
  pushMatrix();
  translate(100, 100);
  scale(c_scale);
